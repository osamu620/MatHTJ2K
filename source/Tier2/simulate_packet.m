function total_bytes_of_packet_header = simulate_packet(hTile, main_header)
M_OFFSET = 1;

SOTlength = uint32(10 + 2);
SODlength = uint32(2);

SOPlength = uint32(4 + 2);
EPHlength = uint32(2);
                                    ...
                                    for jPrecinctX = 0:cr.numprecinctwide - 1
                                        ...
                                        if currentPacket.is_emitted == false
                                            ...
                                            if codingStyle.is_use_SOP() == true
                                                total_bytes_of_packet_header = total_bytes_of_packet_header + SOPlength;
                                            end
                                            % we don't use an empty packet header
                                            simulated_packetHeader.put_bit(1);
                                            for b = 1:cr.num_band
                                                cpb = currentPrecinct.precinct_subbands(b);
                                                generate_packet_header(simulated_packetHeader, cpb, l, true);
                                            end
                                            simulated_packetHeader.flush(codingStyle.is_use_EPH());
                                            total_bytes_of_packet_header = total_bytes_of_packet_header + simulated_packetHeader.num_bytes;
