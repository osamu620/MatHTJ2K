% HT_BLOCK_DECODE_SCRIPT   Generate MEX-function HT_block_decode_mex from
%  HT_block_decode.
%
% Script generated from project 'HT_block_decode.prj' on 23-Jul-2020.
%
...
%% Define argument types for entry-point 'HT_block_decode'.
ARGS = cell(1, 1);
ARGS{1} = cell(2, 1);
...
ARGS{1}{1}.Properties.RD_slope = coder.typeof(uint16(0));
ARGS{1}{1}.Properties.is_reversible = coder.typeof(false);
ARGS{1}{1}.Properties.mq_C = coder.typeof(uint32(0));
ARGS{1}{1}.Properties.mq_A = coder.typeof(uint16(0));
ARGS{1}{1}.Properties.mq_t = coder.typeof(int16(0));
ARGS{1}{1}.Properties.mq_T = coder.typeof(uint8(0));
ARGS{1}{1}.Properties.mq_L = coder.typeof(int32(0));
